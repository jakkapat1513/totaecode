...
class Trader:
    ...
    async def _attach_tpsl_on_start(self):
        ...
        # ใช้ takeProfitMarket / stopMarket, ใส่ workingType='MARK_PRICE', reduceOnly=True
        ...
        try:
            ...
            await loop.run_in_executor(None, lambda: self.ccxt_broker.ex.create_order(
                symbol, type='takeProfitMarket', side=close_side, amount=qty,
                params={**params_base, 'stopPrice': float(tp_price)}
            ))
            await loop.run_in_executor(None, lambda: self.ccxt_broker.ex.create_order(
                symbol, type='stopMarket', side=close_side, amount=qty,
