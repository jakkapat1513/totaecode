...
USE_PERCENT_BALANCE = 0.25            # ใช้ 5% ของ USDT ใน Futures wallet
RSI_PERIOD = 14
RSI_TF = "1m"
RSI_LOW = 30
RSI_HIGH = 70
...
class CCXTBroker:
    ...
    async def connect(self):
        ...
        # Detect hedge (dual-side) mode if possible
        self.is_hedge = False
        try:
            def _dual():
            ...
            try:
                bal = await loop.run_in_executor(None, lambda: self.ex.fetch_balance({'type': 'future'}))
...
class Trader:
    ...
    async def _attach_tpsl_on_start(self):
        ...
        pos_side = "BOTH"
        try:
            bal = await loop.run_in_executor(None, lambda: self.ccxt_broker.ex.fetch_balance({'type': 'future'}))
            positions = (bal.get('info', {}) or {}).get('positions', []) or []
            for p in positions:
    ...
    async def _has_position(self) -> bool:
        """Return True if there is any non-zero futures position for self.symbol."""
        if self.cfg.get("dry_run"):
            return False
        loop = asyncio.get_running_loop()
    ...
    async def _cancel_open_reduce_orders(self):
        """Cancel reduceOnly TP/SL open orders for the current symbol when no position exists."""
        if self.cfg.get("dry_run"):
            return
        loop = asyncio.get_running_loop()
        try:
            opens = await loop.run_in_executor(None, lambda: self.ccxt_broker.ex.fetch_open_orders(self.symbol))
        except Exception as e:
            logging.warning(f"fetch_open_orders error: {e}")
            opens = []
        ...
        for o in opens or []:
            try:
                ...
                if not oid:
                    continue
                await loop.run_in_executor(None, lambda: self.ccxt_broker.ex.cancel_order(oid, self.symbol))
                logging.info(f"ยกเลิก reduceOnly order: {oid}")
            except Exception as e:
                logging.warning(f"ยกเลิกไม่สำเร็จ: {e}")
